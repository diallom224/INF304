...
erreur_programme lire_programme(Programme *prog, char *nom_fichier) {
  ...
  if (fprog == NULL) {
    res.type_err = PROGRAMME_ERREUR_FICHIER;
    return res;
  }
    ...
    while ((i < LGMAX) && (ligne[i] != '\0') && (ligne[i] != '#')) {
      ...
      case '}':
        ...
        if (est_vide(&pile)) {
          ...
          res.type_err = PROGRAMME_ERREUR_FERMETURE_BLOC_EXCEDENTAIRE;
      ...
      default:
        if (est_chiffre(ligne[i])) {
          ...
          res.type_err = PROGRAMME_ERREUR_COMMANDE_INCORRECTE;
          lgligne = strlen(ligne);
          res.ligne = malloc((lgligne + 1) * sizeof(char));
          strcpy(res.ligne, ligne);
          res.num_ligne = numligne;
  ...
  if (est_vide(&pile)) {
    res.type_err = PROGRAMME_OK;
  } else {
    res.type_err = PROGRAMME_ERREUR_BLOC_NON_FERME;
  }
